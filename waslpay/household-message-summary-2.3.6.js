/* WaslPay 2.3.6 — idempotent household message rendering; no payment logic changes. */
'use strict';
const WaslHouseholdMessage236=(()=>{
 const RELEASE='2.3.6';
 ...
 function stamp(root){
  ...
  const badge=body.querySelector('.community-badge');if(badge)setText(badge,'2.3.6 · الأربعاء 23 سبتمبر 2026');
  if(!body.querySelector('[data-release-236]')){
   const p=root.createElement?root.createElement('p'):document.createElement('p');p.dataset.release236='1';
   p.textContent='إصلاح اليوم: تحديث رسائل مصروف البيت أصبح خفيفًا ومتكررًا بأمان؛ لا يعيد كتابة النص نفسه بلا داعٍ، لتقليل الحمل على الهاتف والتابلت عند فتح الرسائل.';
...
if(typeof module!=='undefined'&&module.exports)module.exports=WaslHouseholdMessage236;
if(typeof document!=='undefined'){
 const run=()=>WaslHouseholdMessage236.enhance();
 // DOM replacement/insertions are enough to detect message UI refreshes. Avoid characterData
 // observation because this enhancer itself updates text and could otherwise feed back on itself.
