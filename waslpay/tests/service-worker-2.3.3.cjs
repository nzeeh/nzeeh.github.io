...
(async()=>{
 ...
 test('Precache contains 22 explicit public demo assets',()=>assert.equal(items.size,22));
 test('2.3.3 household message summary included at exact version',()=>assert(items.has(base+'household-message-summary-2.3.3.js?v=2.3.3')));
 test('2.3.2 accessibility patch preserved',()=>assert(items.has(base+'accessibility-2.3.2.js?v=2.3.2')));
 ...
 test('Old WaslPay caches cleaned without touching another project',()=>assert.deepEqual(deleted,['waslpay-demo-shell-old','waslpay-demo-shell-2.3.2']));
 test('Worker claims clients after activation',()=>assert.equal(claimed,true));
 network=false;
 const offline=await(await fetchEvent(base+'?v=2.3.3',{mode:'navigate'})).text();test('Offline navigation query gets shell',()=>assert.equal(offline,'OFFLINE_DEMO'));
 const patch=await(await fetchEvent(base+'household-message-summary-2.3.3.js?v=2.3.3')).text();test('Exact new summary script available offline',()=>assert.equal(patch,'STATIC'));
 test('Future summary script never gets stale copy',()=>assert.equal(fetchEvent(base+'household-message-summary-2.3.3.js?v=FUTURE'),null));
 test('Unversioned summary script bypasses cache',()=>assert.equal(fetchEvent(base+'household-message-summary-2.3.3.js'),null));
