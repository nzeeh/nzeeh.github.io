...
(async()=>{
 ...
 await test('2.3.6 household JS is precached',()=>assert.ok(items.has(base+'household-message-summary-2.3.6.js?v=2.3.6')));
 await test('existing household CSS remains precached',()=>assert.ok(items.has(base+'household-message-summary-2.3.5.css?v=2.3.5')));
 await test('old 2.3.5 household JS is absent',()=>assert.ok(!items.has(base+'household-message-summary-2.3.5.js?v=2.3.5')));
 handlers.activate({waitUntil:p=>pending=p});await pending;
 await test('old WaslPay cache cleaned without touching unrelated project',()=>assert.deepEqual(deleted,['waslpay-demo-shell-2.3.5']));
 await test('new worker claims clients',()=>assert.equal(claimed,true));
 network=false;
 await test('offline navigation falls back to cached shell',async()=>assert.equal(await (await event(base+'?v=2.3.6',{mode:'navigate'})).text(),'OFFLINE_DEMO'));
 await test('2.3.6 JS works offline',async()=>assert.equal(await (await event(base+'household-message-summary-2.3.6.js?v=2.3.6')).text(),'STATIC'));
 await test('household CSS works offline',async()=>assert.equal(await (await event(base+'household-message-summary-2.3.5.css?v=2.3.5')).text(),'STATIC'));
 await test('future JS version is not replaced by stale cache',()=>assert.equal(event(base+'household-message-summary-2.3.6.js?v=FUTURE'),null));
 await test('unversioned household JS bypasses cache',()=>assert.equal(event(base+'household-message-summary-2.3.6.js'),null));
