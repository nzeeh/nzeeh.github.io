...
test('invalid negative snapshot is ignored instead of guessed',()=>assert.equal(H.status({kind:'householdPay',receipt:{householdLeft:-1,householdLimit:1000}}),null));
...
test('low percent rounds for readable warning',()=>assert.equal(H.status(tx(155,1000)).percent,16));
test('zero is exhausted',()=>assert.deepEqual(H.status(tx(0,1000)),{level:'exhausted',left:0,limit:1000,percent:0}));
test('normal summary preserves remaining and total',()=>assert.equal(H.summary(tx(700,3000),money),'متبقي مصروف البيت اليوم: 700 من 3,000 ر.ي'));
test('low summary is explicit and includes amount',()=>assert.equal(H.summary(tx(500,3000),money),'تنبيه: بقي 17٪ من مصروف البيت اليوم · المتبقي 500 من 3,000 ر.ي'));
test('exhausted summary is unambiguous',()=>assert.equal(H.summary(tx(0,3000),money),'نفد مصروف البيت لليوم'));
