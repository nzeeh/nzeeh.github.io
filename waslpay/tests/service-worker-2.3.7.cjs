...
(async()=>{
 ...
 await test('precache contains 24 explicit public assets',()=>assert.equal(items.size,24));
 await test('2.3.7 household JS is precached',()=>assert.ok(items.has(base+'household-message-summary-2.3.7.js?v=2.3.7')));
 await test('2.3.7 safe-area CSS is precached',()=>assert.ok(items.has(base+'safe-area-2.3.7.css?v=2.3.7')));
 await test('old 2.3.6 household JS is absent',()=>assert.ok(!items.has(base+'household-message-summary-2.3.6.js?v=2.3.6')));
 handlers.activate({waitUntil:p=>pending=p});await pending;
 await test('old WaslPay cache cleaned without touching unrelated project',()=>assert.deepEqual(deleted,['waslpay-demo-shell-2.3.6']));
 await test('new worker claims clients',()=>assert.equal(claimed,true));
 network=false;
 await test('offline navigation falls back to cached shell',async()=>assert.equal(await (await event(base+'?v=2.3.7',{mode:'navigate'})).text(),'OFFLINE_DEMO'));
 await test('2.3.7 JS works offline',async()=>assert.equal(await (await event(base+'household-message-summary-2.3.7.js?v=2.3.7')).text(),'STATIC'));
 await test('safe-area CSS works offline',async()=>assert.equal(await (await event(base+'safe-area-2.3.7.css?v=2.3.7')).text(),'STATIC'));
 await test('future JS version is not replaced by stale cache',()=>assert.equal(event(base+'household-message-summary-2.3.7.js?v=FUTURE'),null));
 await test('unversioned household JS bypasses cache',()=>assert.equal(event(base+'household-message-summary-2.3.7.js'),null));
