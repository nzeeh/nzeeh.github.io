/* WaslPay 2.3.5 — household message context and low-budget warning; no payment logic changes. */
'use strict';
const WaslHouseholdMessage235=(()=>{
 const RELEASE='2.3.5';
 ...
 function applyLevel(el,st){
  if(!el)return;el.classList.remove('low','exhausted');
 ...
 function enhanceCards(root,state,D){
  root.querySelectorAll('#messagesBody .message-card[data-id]').forEach(card=>{
   ...
   n.className='message-meta household-message-summary';n.textContent=text;applyLevel(n,st);
  });
 }
 function enhanceDetail(root,state,D){
  ...
  n.className='household-message-detail';n.textContent=text;applyLevel(n,st);
 }
 function enhanceLive(root,state,D){
  ...
  if(extra&&!text.textContent.includes(extra))text.textContent=text.textContent+' · '+extra;
 ...
 function stamp(root){
  ...
  const badge=body.querySelector('.community-badge');if(badge)badge.textContent='2.3.5 · الثلاثاء 22 سبتمبر 2026';
  if(!body.querySelector('[data-release-235]')){
   const p=root.createElement?root.createElement('p'):document.createElement('p');p.dataset.release235='1';
   p.textContent='تحسين اليوم: رسائل مصروف البيت تنبه بوضوح عند بقاء 20٪ أو أقل من الميزانية اليومية، وتوضح نفادها عند الصفر، دون تغيير أي خصم أو حد مالي.';
 ...
 return {RELEASE,snapshot,status,summary,read,txById,applyLevel,enhanceCards,enhanceDetail,enhanceLive,stamp,enhance};
})();
if(typeof module!=='undefined'&&module.exports)module.exports=WaslHouseholdMessage235;
if(typeof document!=='undefined'){
 const run=()=>WaslHouseholdMessage235.enhance();
