...
(() => {
 ...
 const screen = $('screen'), version = '2.1.1';
 ...
 if('serviceWorker' in navigator && (location.protocol==='https:' || ['localhost','127.0.0.1'].includes(location.hostname))){
  window.addEventListener('load',async()=>{
   try{
    ...
    // Check for new releases when a suspended installed app becomes visible again.
    // Throttle checks and never reload a form without an explicit update action.
    let lastCheck=0;
    const checkUpdate=()=>{
     if(navigator.onLine===false||document.visibilityState==='hidden'||Date.now()-lastCheck<60000)return;
